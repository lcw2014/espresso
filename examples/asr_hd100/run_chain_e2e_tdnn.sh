...
if [ ${stage} -le 6 ]; then
  ...
  CUDA_VISIBLE_DEVICES=$free_gpu python3 ../../fairseq_cli/train.py data/chain_e2e --task speech_recognition_hybrid --seed 1 \
    ...
    --arch speech_tdnn_wsj --criterion lattice_free_mmi --criterion-name lattice_free_mmi --num-targets $num_targets \
...
if [ ${stage} -le 7 ]; then
  ...
  for dataset in $test_set; do
    (
      ...
      steps/lmrescore.sh --cmd "$decode_cmd" --self-loop-scale 1.0 --mode 3 data/lang_nosp_test_{tgsmall,tg} \
        data/${dataset}_hires $dir/decode_{tgsmall,tg}_${data_affix} || exit 1
  ...
  for dataset in $test_set; do
    data_affix=$(echo $dataset | sed s/test_//)
    for x in $dir/decode_{tgsmall_${data_affix},tg_${data_affix},tgmed_${data_affix},tgmed_${data_affix}_fg}; do
